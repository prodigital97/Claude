...
function doGet(e) {
  var p = (e && e.parameter) || {};
  // Google OAuth redirects back here with ?code=...&state=... after the user consents.
  if (p.code || p.error) return handleGoogleOAuthCallback(p);
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'googleAuthUrl':        data = handleGoogleAuthUrl(body);        break;
      case 'googleAuthStatus':     data = handleGoogleAuthStatus(body);     break;
      case 'googleAuthDisconnect': data = handleGoogleAuthDisconnect(body); break;
      case 'googleSyncTasks':      data = handleGoogleSyncTasks(body);      break;
      case 'googleSyncGoals':      data = handleGoogleSyncGoals(body);      break;
      case 'googleSyncJourney':    data = handleGoogleSyncJourney(body);    break;
...
var LIST_KINDS = {
  ...
  goal:    { sheet: 'Goals',    fields: ['title', 'status', 'note', 'due'] }
};
...
/* ============================================================================
 *  GOOGLE CALENDAR — per-user OAuth + two-way-capable sync (Tasks, Goals, 75 Hard
 *  journey). Each user connects THEIR OWN Google account (the script's own
 *  "execute as me" identity has no bearing here — CalendarApp would only reach
 *  the deploying account's calendar, so real per-user access needs OAuth).
 *
 *  Setup (see README): create an OAuth Client ID in Google Cloud Console, then
 *  set GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET in Script Properties.
 * ========================================================================== */

var GOOGLE_AUTH_SHEET = 'GoogleAuth';
var GOOGLE_AUTH_HEADERS = ['username', 'accessToken', 'refreshToken', 'expiresAt', 'calendarId', 'email', 'connectedAt', 'updatedAt'];
var GOOGLE_CALENDAR_NAME = 'ATLAS';
var GOOGLE_OAUTH_SCOPE = 'https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/userinfo.email';

function escHtml(s) { return String(s).replace(/[&<>"]/g, function (c) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]; }); }

function handleGoogleAuthUrl(body) {
  var user = authUser(body);
  var clientId = PropertiesService.getScriptProperties().getProperty('GOOGLE_CLIENT_ID');
  if (!clientId) throw new Error('Google Calendar isn\'t set up yet (missing GOOGLE_CLIENT_ID script property).');
  var state = Utilities.getUuid();
  CacheService.getScriptCache().put('gauth_state:' + state, user.username, 600);
  ...
  return { url: url, redirectUri: redirectUri };
}

// Handles the redirect Google sends back to THIS script's own /exec URL after consent.
function handleGoogleOAuthCallback(p) {
  var html;
  try {
    if (p.error) throw new Error('Google said: ' + p.error);
    var state = String(p.state || '');
    var username = state ? CacheService.getScriptCache().get('gauth_state:' + state) : null;
    if (!username) throw new Error('This link expired — go back to ATLAS and tap Connect again.');
    CacheService.getScriptCache().remove('gauth_state:' + state);
    ...
    var tokResp = UrlFetchApp.fetch('https://oauth2.googleapis.com/token', {
      ...
      payload: { code: String(p.code), client_id: clientId, client_secret: clientSecret, redirect_uri: redirectUri, grant_type: 'authorization_code' }
    });
    ...
    var email = '';
    try {
      var uiResp = UrlFetchApp.fetch('https://www.googleapis.com/oauth2/v2/userinfo', { headers: { Authorization: 'Bearer ' + tok.access_token }, muteHttpExceptions: true });
      var ui = JSON.parse(uiResp.getContentText() || '{}'); email = ui.email || '';
    } catch (e) { /* non-fatal — email is just for display */ }

    googleSaveAuth(username, {
      accessToken: tok.access_token, refreshToken: tok.refresh_token || '',
      expiresAt: Date.now() + (Number(tok.expires_in) || 3000) * 1000, email: email
    });
    try { googleEnsureCalendar(username); } catch (e) { /* user can still retry via Sync */ }

    html = '<div style="font-size:44px">✅</div><h2>Google Calendar connected</h2>' +
      '<p>' + (email ? escHtml(email) + ' is now linked to ATLAS.' : 'You\'re connected.') + '</p>' +
      '<p style="opacity:.6">Go back to ATLAS and tap "Check connection".</p>';
  } catch (err) {
    html = '<div style="font-size:44px">⚠️</div><h2>Couldn\'t connect</h2><p>' + escHtml(String(err && err.message || err)) + '</p>';
  }
  return HtmlService.createHtmlOutput(
    '<html><body style="font-family:-apple-system,sans-serif;background:#0a0b0e;color:#f4f5f7;text-align:center;padding:70px 24px;">' + html + '</body></html>'
  );
}

function googleAuthSheet() { return getSheet(GOOGLE_AUTH_SHEET, GOOGLE_AUTH_HEADERS); }
function googleFindAuthRow(username) {
  var sheet = googleAuthSheet();
  var values = sheet.getDataRange().getValues();
  var idx = colIndex(GOOGLE_AUTH_HEADERS);
  for (var i = 1; i < values.length; i++) {
    if (normalizeUsername(values[i][idx.username]) === username) return { row: i + 1, values: values[i] };
  }
  return null;
}
function googleSaveAuth(username, patch) {
  var sheet = googleAuthSheet();
  var idx = colIndex(GOOGLE_AUTH_HEADERS);
  var found = googleFindAuthRow(username);
  var now = new Date().toISOString();
  if (found) {
    Object.keys(patch).forEach(function (k) { if (idx[k] !== undefined) sheet.getRange(found.row, idx[k] + 1).setValue(patch[k]); });
    sheet.getRange(found.row, idx.updatedAt + 1).setValue(now);
  } else {
    var rec = { username: username, accessToken: '', refreshToken: '', expiresAt: 0, calendarId: '', email: '', connectedAt: now, updatedAt: now };
    Object.keys(patch).forEach(function (k) { rec[k] = patch[k]; });
    sheet.appendRow(GOOGLE_AUTH_HEADERS.map(function (h) { return rec[h]; }));
  }
}
function googleGetAuth(username) {
  var found = googleFindAuthRow(username);
  if (!found) return null;
  var idx = colIndex(GOOGLE_AUTH_HEADERS), r = found.values;
  return {
    accessToken: String(r[idx.accessToken] || ''), refreshToken: String(r[idx.refreshToken] || ''),
    expiresAt: Number(r[idx.expiresAt]) || 0, calendarId: String(r[idx.calendarId] || ''), email: String(r[idx.email] || '')
  };
}

function handleGoogleAuthStatus(body) {
  var user = authUser(body);
  var a = googleGetAuth(user.username);
  return { connected: !!(a && a.refreshToken), email: a ? a.email : '' };
}
function handleGoogleAuthDisconnect(body) {
  var user = authUser(body);
  var a = googleGetAuth(user.username);
  if (a && a.accessToken) {
    try { UrlFetchApp.fetch('https://oauth2.googleapis.com/revoke?token=' + encodeURIComponent(a.accessToken), { method: 'post', muteHttpExceptions: true }); } catch (e) {}
  }
  var found = googleFindAuthRow(user.username);
  if (found) googleAuthSheet().deleteRow(found.row);
  return { disconnected: true };
}

// Valid access token for this user, refreshing via the stored refresh_token if needed.
function googleEnsureAccessToken(username) {
  var a = googleGetAuth(username);
  if (!a || !a.refreshToken) throw new Error('Connect Google Calendar first (Settings → Google Calendar).');
  if (a.accessToken && a.expiresAt > Date.now() + 60000) return a.accessToken;
  var clientId = PropertiesService.getScriptProperties().getProperty('GOOGLE_CLIENT_ID');
  var clientSecret = PropertiesService.getScriptProperties().getProperty('GOOGLE_CLIENT_SECRET');
  var resp = UrlFetchApp.fetch('https://oauth2.googleapis.com/token', {
    method: 'post', muteHttpExceptions: true,
    payload: { refresh_token: a.refreshToken, client_id: clientId, client_secret: clientSecret, grant_type: 'refresh_token' }
  });
  var tok = JSON.parse(resp.getContentText() || '{}');
  if (tok.error) throw new Error('Your Google Calendar connection expired — reconnect it in Settings.');
  googleSaveAuth(username, { accessToken: tok.access_token, expiresAt: Date.now() + (Number(tok.expires_in) || 3000) * 1000 });
  return tok.access_token;
}

// Thin wrapper around one Calendar API v3 call.
function googleCal(accessToken, method, pathAndQuery, bodyObj) {
  var opts = { method: method, muteHttpExceptions: true, headers: { Authorization: 'Bearer ' + accessToken } };
  if (bodyObj !== undefined) { opts.contentType = 'application/json'; opts.payload = JSON.stringify(bodyObj); }
  var resp = UrlFetchApp.fetch('https://www.googleapis.com/calendar/v3/' + pathAndQuery, opts);
  var code = resp.getResponseCode();
  var data = {};
  try { data = JSON.parse(resp.getContentText() || '{}'); } catch (e) {}
  if (code >= 400) throw new Error((data.error && data.error.message) || ('Google Calendar error ' + code));
  return data;
}

function googleEnsureCalendar(username) {
  var a = googleGetAuth(username);
  if (a && a.calendarId) return a.calendarId;
  var token = googleEnsureAccessToken(username);
  var list = googleCal(token, 'get', 'users/me/calendarList?minAccessRole=owner');
  var existing = (list.items || []).filter(function (c) { return c.summary === GOOGLE_CALENDAR_NAME; })[0];
  var calendarId = existing ? existing.id : googleCal(token, 'post', 'calendars', {
    summary: GOOGLE_CALENDAR_NAME, description: 'Created by ATLAS — your Tasks, Goals & 75 Hard journey.'
  }).id;
  googleSaveAuth(username, { calendarId: calendarId });
  return calendarId;
}

// Every event we've previously tagged with this atlasKind, keyed by our own atlasId.
function googleListTagged(token, calendarId, kind) {
  var out = {};
  var q = 'calendars/' + encodeURIComponent(calendarId) + '/events' +
    '?privateExtendedProperty=' + encodeURIComponent('atlasApp=atlas75hard') +
    '&privateExtendedProperty=' + encodeURIComponent('atlasKind=' + kind) +
    '&maxResults=2500&showDeleted=false&singleEvents=true';
  var data = googleCal(token, 'get', q);
  (data.items || []).forEach(function (ev) {
    var id = ev.extendedProperties && ev.extendedProperties.private && ev.extendedProperties.private.atlasId;
    if (id) out[id] = ev;
  });
  return out;
}
function googleAddDays(dateStr, n) {
  var d = new Date(dateStr + 'T00:00:00');
  d.setDate(d.getDate() + n);
  return Utilities.formatDate(d, Session.getScriptTimeZone(), 'yyyy-MM-dd');
}
function googleEventBody(summary, description, dateStr, kind, atlasId) {
  return {
    summary: summary, description: description,
    start: { date: dateStr }, end: { date: googleAddDays(dateStr, 1) },
    extendedProperties: { private: { atlasApp: 'atlas75hard', atlasKind: kind, atlasId: String(atlasId) } }
  };
}
function googleEventUnchanged(ev, body) {
  return ev.summary === body.summary && ev.start && ev.start.date === body.start.date && String(ev.description || '') === String(body.description || '');
}

// Diff-syncs `items` ({id, date, summary, description}) into this kind's tagged events:
// creates missing ones, patches changed ones, deletes ones with no matching item anymore,
// and skips ones that are already correct — so re-syncing is fast and idempotent.
function googleSyncItems(username, kind, items) {
  var token = googleEnsureAccessToken(username);
  var calendarId = googleEnsureCalendar(username);
  var existing = googleListTagged(token, calendarId, kind);
  var seen = {}, created = 0, updated = 0, deleted = 0, skipped = 0;
  items.forEach(function (it) {
    seen[it.id] = true;
    var body = googleEventBody(it.summary, it.description || '', it.date, kind, it.id);
    var ev = existing[it.id];
    if (!ev) { googleCal(token, 'post', 'calendars/' + encodeURIComponent(calendarId) + '/events', body); created++; }
    else if (!googleEventUnchanged(ev, body)) { googleCal(token, 'patch', 'calendars/' + encodeURIComponent(calendarId) + '/events/' + encodeURIComponent(ev.id), body); updated++; }
    else skipped++;
  });
  Object.keys(existing).forEach(function (id) {
    if (!seen[id]) { googleCal(token, 'delete', 'calendars/' + encodeURIComponent(calendarId) + '/events/' + encodeURIComponent(existing[id].id)); deleted++; }
  });
  return { created: created, updated: updated, deleted: deleted, skipped: skipped, total: items.length };
}

function handleGoogleSyncTasks(body) {
  var user = authUser(body);
  var sheet = listSheetFor('task');
  var rows = sheet.getDataRange().getValues();
  var idx = colIndex(listHeaders('task'));
  var items = [];
  for (var i = 1; i < rows.length; i++) {
    if (normalizeUsername(rows[i][idx.username]) !== user.username) continue;
    var due = String(rows[i][idx.due] || '');
    if (!due) continue;
    var done = toBool(rows[i][idx.done]);
    items.push({
      id: String(rows[i][idx.id]), date: formatDate(due),
      summary: (done ? '✅ ' : '') + String(rows[i][idx.title] || 'Task'),
      description: 'Priority: ' + String(rows[i][idx.priority] || 'normal') + '\nSynced from ATLAS Tasks.'
    });
  }
  return googleSyncItems(user.username, 'task', items);
}
function handleGoogleSyncGoals(body) {
  var user = authUser(body);
  var sheet = listSheetFor('goal');
  var rows = sheet.getDataRange().getValues();
  var idx = colIndex(listHeaders('goal'));
  var items = [];
  for (var i = 1; i < rows.length; i++) {
    if (normalizeUsername(rows[i][idx.username]) !== user.username) continue;
    var due = String(rows[i][idx.due] || '');
    ...
    items.push({
      id: String(rows[i][idx.id]), date: formatDate(due),
      summary: '🎯 ' + String(rows[i][idx.title] || 'Goal'),
      description: String(rows[i][idx.note] || '') + '\nStatus: ' + String(rows[i][idx.status] || '') + '\nSynced from ATLAS Goals.'
    });
  }
  return googleSyncItems(user.username, 'goal', items);
}
function handleGoogleSyncJourney(body) {
  var user = authUser(body);
  var usheet = getSheet(USERS_SHEET, USER_HEADERS);
  var found = findUserRow(usheet, user.username);
  if (!found) throw new Error('User not found.');
  var uidx = colIndex(USER_HEADERS);
  var startDate = formatDate(found.values[uidx.startDate]);

  var prof = getProfile(user.username);
  var mode = prof.mode === 'soft' ? 'soft' : 'hard';
  var target = Number(prof.softTarget) || 70;

  var logRows = getSheet(LOGS_SHEET, LOG_HEADERS).getDataRange().getValues();
  var lidx = colIndex(LOG_HEADERS);
  var byDate = {};
  for (var i = 1; i < logRows.length; i++) {
    if (normalizeUsername(logRows[i][lidx.username]) !== user.username) continue;
    byDate[formatDate(logRows[i][lidx.date])] = logFromRow(logRows[i], lidx);
  }
  var today = todayStr();
  var items = [];
  for (var n = 1; n <= CHALLENGE_LENGTH; n++) {
    var date = googleAddDays(startDate, n - 1);
    var log = byDate[date];
    var met = log ? logGoalMet(log, mode, target) : false;
    var flag = met ? ' ✅' : (date < today ? ' ❌' : '');
    items.push({
      id: String(n), date: date, summary: '💪 75 Hard — Day ' + n + '/' + CHALLENGE_LENGTH + flag,
      description: 'Synced from your ATLAS 75 Hard journey.'
    });
  }
  return googleSyncItems(user.username, 'journeyday', items);
}
