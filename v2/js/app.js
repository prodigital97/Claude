...
(function () {
  ...
  /* ---------------- state ---------------- */
  var state = {
    token: localStorage.getItem('atlasv2_token') || '',
    username: localStorage.getItem('atlasv2_user') || '',
    user: null,
    logs: [],
    today: null,
    profile: {},
  ...
  var FX = {
    haptics: localStorage.getItem('atlasv2_haptics') !== '0', // default on
    sound: localStorage.getItem('atlasv2_sound') !== '0',     // default on
    actx: null,
    ensure: function () {
    ...
    set: function (key, on) {
      this[key] = on;
      localStorage.setItem(key === 'haptics' ? 'atlasv2_haptics' : 'atlasv2_sound', on ? '1' : '0');
  ...
  function db() { return JSON.parse(localStorage.getItem('atlasv2_demo') || '{"users":{},"logs":{}}'); }
  function saveDb(d) { localStorage.setItem('atlasv2_demo', JSON.stringify(d)); }
  ...
  function authSubmit(action, payload, busy) {
    ...
    api(action, payload).then(function (data) {
      ...
      localStorage.setItem('atlasv2_token', state.token);
      localStorage.setItem('atlasv2_user', state.username);
      authMsg('');
      return loadState();
  ...
  function loadState() {
    return api('getState', {}).then(function (data) {
      ...
      lastXpLevel = levelInfo(xpTotals().total).level;   // v2 lab: seed baseline once data is ready
      cacheState();
    });
  }
  function cacheState() {
    localStorage.setItem('atlasv2_cache', JSON.stringify({ user: state.user, logs: state.logs }));
  }
  ...
  function renderToday() {
    ...
    chRules().forEach(function (rule) {
      ...
      row.addEventListener('click', function () {
        ...
        var before = dayXp(d).total;                             // v2 lab: XP delta
        ruleToggle(d, rule);
  ...
  function toggleTask(t) {
    ...
    gamifyAfterToggle(delta);   // XP floater + level-up (v2 lab)
  }
  ...
  /* ---------------- Stats ---------------- */
  function statsCacheKey() { return 'atlasv2_stats_' + (state.username || ''); }
  ...
  function acctChangeUsername() {
    ...
    api('changeUsername', { newUsername: newName, password: pw }).then(function (d) {
      ...
      localStorage.setItem('atlasv2_token', state.token); localStorage.setItem('atlasv2_user', state.username);
      cacheState();
  ...
  function deleteAccount() {
    ...
    api('deleteAccount', { password: pw }).then(function () {
      localStorage.removeItem('atlasv2_token');
      localStorage.removeItem('atlasv2_user');
      localStorage.removeItem('atlasv2_cache');
  ...
  function getFoodStats() {
    try { return JSON.parse(localStorage.getItem('atlasv2_foodstats') || '{}'); } catch (e) { return {}; }
  }
  function addRecent(food) {
    ...
    localStorage.setItem('atlasv2_foodstats', JSON.stringify(m));
  }
  ...
  function bindDietEvents() {
    ...
    $('#ai-scan').checked = localStorage.getItem('atlasv2_aiscan') === '1';
    function syncTwoImg() {
    ...
    $('#ai-scan').addEventListener('change', function () { localStorage.setItem('atlasv2_aiscan', this.checked ? '1' : '0'); syncTwoImg(); });
    syncTwoImg();
  ...
  function currentTheme() { return localStorage.getItem('atlasv2_theme') || CFG.DEFAULT_THEME || 'midnight'; }
  function applyTheme(id) {
    ...
    localStorage.setItem('atlasv2_theme', t.id);
  }
  ...
  function renderHome() {
    ...
    // Gamification (v2 lab) — level + XP progress bar, derived from history.
    var xt = xpTotals();
  ...
  function renderWaterApp() {
    ...
    // Loss mechanic (v2 lab): an untouched jar TODAY reads as dry/wilted — the
    // cost of inaction is visible on open, no notification needed.
  ...
  /* ================= Digital Detox (Mind) ================= */
  function detoxKey() { return 'atlasv2_detox_' + (state.username || ''); }
  ...
  function moneyCacheKey() { return 'atlasv2_money_' + (state.username || ''); }
  ...
  /* ---------------- AI Coach chat ---------------- */
  function coachKey() { return 'atlasv2_coach_' + (state.coachDomain || 'hard') + '_' + (state.username || ''); }
  ...
  function boot() {
    ...
    if (state.token && state.username) {
      ...
      var cache = JSON.parse(localStorage.getItem('atlasv2_cache') || 'null');
      if (cache && cache.user) {
